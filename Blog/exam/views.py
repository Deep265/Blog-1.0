...
class TestDetailView(LoginRequiredMixin,DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (datetime.datetime.now().strftime("%H:%M:%S") >= datetime.time(hour=int(test.test_time[0:2]),minute=int(test.test_time[3:5]),second=0).strftime("%H:%M:%S")):
            context['start'] = 0

        if (datetime.datetime.now().strftime("%H:%M:%S") <= datetime.time(hour=int(test.test_end_time[0:2]),minute=int(test.test_end_time[3:5]),second=0).strftime("%H:%M:%S")):
            context['end'] = 0
